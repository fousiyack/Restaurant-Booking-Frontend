...
function RestBookingHistory() {
  ...
  console.log(restaurantId, "is this workingggggggggggg");
  ...
  const fetchBookings = async () => {
    try {
      console.log(restaurantId, "inside........");
      const response = await axios.get(
        `${BASE_URL}/restaurant/restBooking-history/${restaurantId}/`
      );
    ...
    <div className="flex flex-1 flex-col md:flex-row lg:flex-row mx-2">
      <div className="mb-2 border-solid border-gray-300 rounded border shadow-sm w-full">
        <div className="flex justify-between items-center">
          ...
          <div className="flex">
            <label className="mr-2">Search User:</label>
            <input
              type="text"
              className="appearance-none block bg-gray-200 text-gray-darker border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray"
              id="Search"
              name="Search"
              value={search}
              ...
              <tbody>
                {filteredData.map((booking) => (
                  <tr key={booking.id}>
                    ...
                    <td className="border px-4 py-2">
                      {booking.timeId.start_time.slice(0, -3)}
                    </td>
                    <td className="border px-4 py-2">
                      {booking.timeId.end_time.slice(0, -3)}
                    </td>
                    <td className="border px-4 py-2">
...
export default RestBookingHistory;
