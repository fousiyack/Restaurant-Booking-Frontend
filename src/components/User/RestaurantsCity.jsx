...
const RestaurantsCity = () => {
  ...
  const fetchAndFilterRestaurants = async (selectedCityId) => {
    try {
      const response = await axios.get(`${BASE_URL}/restaurant/RestListCity/${selectedCityId}`);
  ...
  useEffect(() => {
    fetchAndFilterRestaurants(city).then((restaurants) => {
